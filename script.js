// Thêm hai hàm chuyển đổi vào đầu script
    window.convertTTForStorage = function(tt) {
        return String(tt).replace(/\./g, ',');
    };

    window.convertTTForDisplay = function(tt) {
        return String(tt).replace(/,/g, '.');
    };

// === BẮT ĐẦU MÃ MỚI: HÀM HỖ TRỢ SỐ LA MÃ ===
    window.numberToRoman = function(num) {
        const romanMap = [
    ...
    window.romanToNumber = function(roman) {
        if (!roman) return 0;
...
// Kiểm tra xem config có tồn tại không để tránh lỗi
if (!firebaseConfig) {
...
const parseNumber = (str) => {
    ...
    window.getParentCollectionName = function() {
 ...
 */
window.normalizeTT = function(rawTT) {
    if (!rawTT) return '';
    return String(rawTT)
 ...
 */
	window.detectLevel = function(tt) {
	...
	window.buildImportUpdates_Merge = function(jsonData, headers) {
...
// Hàm gọi lại sau khi nhận token
    // Hàm gọi lại sau khi nhận token
    window.tokenResponseCallback = function(resp) {
   ...
   // Hàm xử lý việc yêu cầu xác thực
    window.requestGoogleAuth = async function() {
...
window.recalculateAllCostsAndReload = async function(showConfirm = true) {
    ...
    try {
        ...
        // Tìm đoạn khai báo aggregateFromChildren bên trong recalculateAllCostsAndReload
const aggregateFromChildren = (parentNode, childCollectionName) => {
    ...
    if (parentNode[childCollectionName]) {
        for (const key in parentNode[childCollectionName]) {
            ...
            // === [LOGIC MỚI]: Bỏ qua tính toán nếu Cấp độ là PS ===
            const isPS = String(child.capDo || '').trim().toUpperCase() === 'PS';
            ...
            // Logic kiểm tra "đã thực hiện" vẫn giữ nguyên để hiển thị trạng thái
 ...
 */
	window.getCleanupFieldsForParentNode = function(path) {
    return {
        [`${path}/donVi`]: null,
        [`${path}/soLuong`]: null,
        [`${path}/donGia`]: null,
...
window.saveTasks = async function() {
  ...
  /// === CẬP NHẬT: HÀM saveNewTask (Thêm logic null cho Tần suất) ===
...
// === KẾT THÚC hàm cho nút sửa ===
   ...
   /**
     ...
     */
    window.updateAncestors = function(currentPath) {
 ...
 */
	window.findMaxExecutions = function(nodes) {
    let maxExec = 0;
    if (!nodes) return 0;
 ...
 */
	...
	window.processJustificationForExcel = function(justificationNode, parentTT, level, ctx) {
 ...
 */
	window.processNodeForExcel = function(node, path, level, ctx) {
    const r = ctx.currentRow;
    let childRows = [];
 ...
 */
   window.generateWorksheetWithFormulas = async function(includeJustifications) {
    const parentQuery = ref(db, getParentCollectionName());
    const parentSnapshot = await get(parentQuery);
    ...
    // --- THÊM CỘT 'CP Cấp 1' ---
    const baseHeaders = [
        'TT', 'Nội dung công việc', 'Tag Name', 'ĐV th.hiện', 'KH năm trước', 'Thực hiện năm trước', 
    ...
    const colNameMap = {
        'TT': 'TT', 'Nội dung công việc': 'NOI_DUNG',
		'Tag Name': 'TAG_NAME',
...
// === HÀM XUẤT EXCEL (CẬP NHẬT: EXCEL 1 ĐỦ CỘT NHƯNG KHÔNG GIẢI TRÌNH) ===
    ...
    try {
        ...
        const processNode = (node, path, parentTT = '', level = 0) => {
			...
			let tgBatDauVal = node.tgBatDau;	
                ...
                const row = {
                    ...
                    'Nội dung công việc': node.noiDung || '',
            ...
            if (activeExportType === 'full' && node.justifications) {
                ...
                justifications.forEach(just => {
                    const justRow = {
                        ...
                        'Nội dung công việc': just.noiDung || '',
						'Tag Name': '',
                    ...
                    if (just.hasSubJustifications && just.subJustifications) {
                        ...
                        subJustifications.forEach(subJust => {
                            const subJustRow = {
                                ...
                                'Nội dung công việc': subJust.noiDung || '',
                                'ĐV th.hiện': '', 'KH năm trước': '', 'Thực hiện năm trước': '', 'Tần suất TH': '',
        ...
        const baseHeadersFull = ['TT', 'Nội dung công việc', 'Tag Name', 'ĐV th.hiện', 'KH năm trước', 'Thực hiện năm trước', 'Tần suất TH', 'Đ.vị tính', 'Số lượng', 'Đơn giá (GT)', 'Chi phí KH', 'Chi phí thực hiện', 'Kế hoạch còn lại', 'Năm phân bổ', 'CP phân bổ', 'Cấp độ', 'CP Cấp 1', 'Thời điểm TH', 'TG hoàn thành', 'Ghi chú'];
...
// === BẮT ĐẦU: HÀM HELPER MỚI ĐỂ TĂNG TT ===
   window.incrementTT = function(displayTT) {
    try {
    ...
    window.initializeGoogleClients = function() {
    ...
    window.ensureGapiAuthenticated = function() {
        return new Promise((resolve, reject) => {
    ...
    // Hàm tính toán chi phí theo cấu trúc cây (Tree)
// Đã tích hợp logic: Nếu task con có capDo là 'PS' thì không cộng dồn lên cha
const calculateCosts = (tree) => {
    tree.forEach(parent => {
        // Khởi tạo biến tổng cho Cấp 1 (Parent)
        let parentChiPhi = 0;
        let parentChiPhiThucHien = 0;
        let parentKhNamTruoc = 0;
        let parentThucHienNamTruoc = 0;
        let parentChiPhiPhanBo = 0;

        (parent.children || []).forEach(child => {
            // Khởi tạo biến tổng cho Cấp 2 (Child)
            let childChiPhi = 0;
            let childChiPhiThucHien = 0;
            let childKhNamTruoc = 0;
            let childThucHienNamTruoc = 0;
            let childChiPhiPhanBo = 0;

            (child.grandchildren || []).forEach(grandchild => {
                // Khởi tạo biến tổng cho Cấp 3 (Grandchild)
                let grandchildChiPhi = 0;
                let grandchildChiPhiThucHien = 0;
                let grandchildKhNamTruoc = 0;
                let grandchildThucHienNamTruoc = 0;
                let grandchildChiPhiPhanBo = 0;

                (grandchild.greatGrandchildren || []).forEach(ggc => {
                    // --- XỬ LÝ CẤP 4 (GreatGrandchild) ---
                    
                    // 1. Kiểm tra xem có Cấp 5 (GreatGreatGrandchildren) không
                    if ((ggc.greatGreatGrandchildren || []).length > 0) {
                        ggc.data.hasChildren = true;
                        
                        // Nếu có con cấp 5, tính tổng từ cấp 5 lên cấp 4
                        let ggcChiPhi = 0;
                        let ggcChiPhiThucHien = 0;
                        // (Thường cấp 5 ít dùng nhưng giữ logic để đảm bảo đầy đủ)
                        ggc.greatGreatGrandchildren.forEach(gggc => {
                             ggcChiPhi += (gggc.data.chiPhi || 0);
                             ggcChiPhiThucHien += (gggc.data.chiPhiThucHien || 0);
                        });
                        ...
                        // Kiểm tra nếu có giải trình (Justifications) thì tính tổng giải trình
                        if (ggc.justifications && ggc.justifications.length > 0) {
                             let justificationChiPhi = 0;
                             ggc.justifications.forEach(j => {
                                 let subCost = j.chiPhi || 0;
                                 // Nếu giải trình có giải trình con
                                 if (j.subJustifications) {
                                     let subTotal = 0;
                                     Object.values(j.subJustifications).forEach(sub => subTotal += (sub.chiPhi || 0));
                                     subCost = subTotal;
                                 }
                                 justificationChiPhi += subCost;
                             });
                             // Gán tổng chi phí giải trình vào chi phí của Task
                             ggc.data.chiPhi = justificationChiPhi; 
                        }
                    }

                    // 2. Tính toán nội bộ cho Cấp 4 (Kế hoạch còn lại)
                    if (ggc.data) {
                        ggc.data.keHoachConLai = (ggc.data.chiPhi || 0) - (ggc.data.chiPhiThucHien || 0);
                    }

                    // 3. === LOGIC QUAN TRỌNG: KIỂM TRA CẤP ĐỘ PS ===
                    // Lấy giá trị cấp độ, xóa khoảng trắng và chuyển về chữ hoa
                    const isGgcPS = String(ggc.data.capDo || '').trim().toUpperCase() === 'PS';

                    // 4. Cộng dồn lên Cấp 3 (Grandchild)
                    // CHỈ CỘNG NẾU KHÔNG PHẢI LÀ 'PS'
                    if (!isGgcPS) {
                        ...
                        grandchildThucHienNamTruoc += (ggc.data.thucHienNamTruoc || 0);
                        grandchildChiPhiPhanBo += (parseFloat(ggc.data.chiPhiPhanBo) || 0);
                    }
                });

                // --- CẬP NHẬT DỮ LIỆU CẤP 3 (Grandchild) ---
                if (grandchild.data) {
                    ...
                    grandchild.data.thucHienNamTruoc = grandchildThucHienNamTruoc;
                    grandchild.data.chiPhiPhanBo = grandchildChiPhiPhanBo;
                    
                    // Tính còn lại cho cấp 3
                    grandchild.data.keHoachConLai = grandchildChiPhi - grandchildChiPhiThucHien;
                }

                // Cộng dồn Cấp 3 lên Cấp 2 (Child)
                // (Ở cấp này chưa áp dụng logic PS, trừ khi bạn muốn mở rộng thêm)
                childChiPhi += grandchildChiPhi;
                childChiPhiThucHien += grandchildChiPhiThucHien;
                childKhNamTruoc += grandchildKhNamTruoc;
                childThucHienNamTruoc += grandchildThucHienNamTruoc;
                childChiPhiPhanBo += grandchildChiPhiPhanBo;
            });

            // --- CẬP NHẬT DỮ LIỆU CẤP 2 (Child) ---
            if (child.data) {
                child.data.chiPhi = childChiPhi;
                child.data.chiPhiThucHien = childChiPhiThucHien;
                child.data.khNamTruoc = childKhNamTruoc;
                child.data.thucHienNamTruoc = childThucHienNamTruoc;
                child.data.chiPhiPhanBo = childChiPhiPhanBo;

                // Tính còn lại cho cấp 2
                child.data.keHoachConLai = childChiPhi - childChiPhiThucHien;
            }

            // Cộng dồn Cấp 2 lên Cấp 1 (Parent)
            parentChiPhi += childChiPhi;
            parentChiPhiThucHien += childChiPhiThucHien;
            parentKhNamTruoc += childKhNamTruoc;
            parentThucHienNamTruoc += childThucHienNamTruoc;
            parentChiPhiPhanBo += childChiPhiPhanBo;
        });
        
        // --- CẬP NHẬT DỮ LIỆU CẤP 1 (Parent) ---
        if (parent.data) {
            parent.data.chiPhi = parentChiPhi;
            parent.data.chiPhiThucHien = parentChiPhiThucHien;
            parent.data.khNamTruoc = parentKhNamTruoc;
            parent.data.thucHienNamTruoc = parentThucHienNamTruoc;
            parent.data.chiPhiPhanBo = parentChiPhiPhanBo;

            // Tính còn lại cho cấp 1
  ...
  const generateNextTT = async (parentPath, level) => {
     ...
     */
    window.clearChildrenCheckboxes = function(parentCheckbox) {
...
const recalculateSingleParentCost = async (parentPath, childrenField = null, fieldsToRecalculate = ['chiPhi', 'chiPhiThucHien', 'keHoachConLai', 'khNamTruoc', 'thucHienNamTruoc', 'chiPhiPhanBo', 'cpCap1']) => {
    ...
    if (isLeafNode) {
        ...
        if (typeof childrenData === 'object' && childrenData !== null) {
            for (const childId in childrenData) {
                if (typeof childrenData[childId] === 'object' && childrenData[childId] !== null) {
                    ...
                    // === [LOGIC MỚI]: Bỏ qua nếu Cấp độ là PS ===
                    const isPS = String(child.capDo || '').trim().toUpperCase() === 'PS';
        ...
        // ... (Phần code update fieldsToRecalculate bên dưới giữ nguyên) ...
        ...
        // Tính lại Kế hoạch còn lại cho cha
...
// Ensure checkAndUpdateDaThucHien is robust
  ...
  const calculateSummaryData = async () => {
    ...
    window.ensureYearCollectionExists = async function(year) {
    ...
    window.openPlanningModeModal = function() {
    ...
    window.activatePlanningModeForSelection = function(selectionId) {
        const taskList = document.getElementById('taskList');
    ...
    // ============================================================
    // TÍNH NĂNG CAO CẤP: PASTE TỪ EXCEL VÀO MODAL
    // ============================================================
    
    document.addEventListener('paste', function(e) {
        // 1. Chỉ kích hoạt khi đang ở trong Modal Dữ liệu (#dataModal) và con trỏ ở trong bảng
        const target = e.target;
        const modalTable = target.closest('#dataModal .modal-table');
        
        if (!modalTable || (!target.matches('input') && !target.matches('select') && !target.matches('textarea'))) {
            return;
        }
        ...
        e.preventDefault(); // Ngăn hành động paste mặc định (dồn cục vào 1 ô)

        // 3. Phân tích dữ liệu (Excel dùng \t cho cột và \n cho dòng)
        // Loại bỏ dòng trống cuối cùng nếu có
        const rows = clipboardData.split(/\r\n|\n|\r/).filter((row, index, array) => {
            return !(index === array.length - 1 && row.trim() === '');
        });
        
        if (rows.length === 0) return;

        // 4. Xác định vị trí con trỏ hiện tại
        const currentRow = target.closest('tr');
        const allRows = Array.from(modalTable.querySelectorAll('tbody tr'));
        const startRowIndex = allRows.indexOf(currentRow);

        // Lấy tất cả các ô nhập liệu trong dòng hiện tại để xác định chỉ số cột
        // Lưu ý: Chỉ lấy các ô input/select/textarea hiển thị được (không lấy hidden)
        const getInputsInRow = (tr) => Array.from(tr.querySelectorAll('input:not([type="hidden"]), select, textarea'));
        
        const inputsInCurrentRow = getInputsInRow(currentRow);
        const startColIndex = inputsInCurrentRow.indexOf(target);
        ...
        // 5. Duyệt qua dữ liệu và đổ vào bảng
        rows.forEach((rowString, i) => {
            ...
            const targetInputs = getInputsInRow(targetRow);

            cells.forEach((cellData, j) => {
                const targetColIndex = startColIndex + j;

                // Kiểm tra thiếu cột (hoặc ô bị disabled)
                if (targetColIndex >= targetInputs.length) {
                    if (i === 0) missingCols++; // Chỉ đếm cột thiếu ở dòng đầu tiên để báo cáo
                    return;
                }

                const inputEl = targetInputs[targetColIndex];

                // Chỉ paste vào ô không bị khóa
                if (inputEl && !inputEl.disabled) {
                    // Xử lý dữ liệu thô: Xóa ngoặc kép nếu Excel thêm vào (khi có xuống dòng)
                    let cleanValue = cellData.trim();
                    ...
                    // QUAN TRỌNG: Kích hoạt sự kiện để các hàm tính toán/định dạng hoạt động
                    inputEl.dispatchEvent(new Event('input', { bubbles: true }));
                    inputEl.dispatchEvent(new Event('change', { bubbles: true }));
                    
                    // Nếu là ô số, trigger thêm focus/blur để format lại tiền tệ (nếu cần)
                    if (inputEl.dataset.field && ['chiPhi', 'donGia', 'soLuong', 'khNamTruoc', 'thucHienNamTruoc'].includes(inputEl.dataset.field)) {
                         // Giả lập hành vi người dùng nhập xong và blur ra ngoài để format số
                         // Logic trong oninput="this.value=formatNumber..." sẽ chạy nhờ Event 'input' ở trên
                    }
                }
            });
        });

        // 6. Cảnh báo nếu thiếu dòng/cột
        if (missingRows > 0 || missingCols > 0) {
            let msg = 'CẢNH BÁO: Dữ liệu Paste vượt quá kích thước bảng!\n';
            if (missingRows > 0) msg += `- Thiếu ${missingRows} dòng bên dưới.\n`;
            if (missingCols > 0) msg += `- Thiếu cột hoặc vượt quá số ô nhập liệu bên phải.\n`;
            msg += '\nVui lòng thêm đủ dòng trước khi Paste hoặc kiểm tra lại vị trí đặt con trỏ.';
            alert(msg);
        }
    });
 ...
 // XỬ LÝ AUTH STATE
  ...
  // KHỞI TẠO ỨNG DỤNG
    window.addEventListener('DOMContentLoaded', async function() { // Thêm async ở đây
		...
		window.startSummaryInterval = function() {
        ...
        window.stopSummaryInterval = function() {
 ...
 */
   window.makeCellEditable = async function(event) {
   // Thêm async để chờ load danh sách
...
parentData.tgBatDau = parentSummary.totalExecutedLeafNodesCap1 || 0;
 ...
 /**
     ...
     */
    window.enableArrowKeyNavigation = function(modalId) {
